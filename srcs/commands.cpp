...
void irc::commands::operator[](irc::client &client_)
{
	...
	if (client_._message._command.empty())
		client_._msg_out = ERR_NOCOMMANDGIVEN(client_);
	else
	{
		commands_iterator func_it = this->_commands.find(client_._message._command);
		if (func_it == this->_commands.end())
...
void irc::commands::nick(irc::client &client_)
{
	...
	if (client_._message._params.empty() || client_._message._params.front().empty())
		...
		client_._msg_out = client_._realname.compare("*") ? ERR_NICKNAMEINUSE(client_) : ERR_NICKCOLLISION(client_);
...
{
	if (client_._message._params.size() < 4)
		client_._msg_out = ERR_NEEDMOREPARAMS(client_);
	else if (client_._realname.compare("*"))
		client_._msg_out = ERR_ALREADYREGISTRED(client_);
	else
...
{
	if (client_._message._params.empty())
		client_._msg_out = ERR_NOORIGIN(client_);
