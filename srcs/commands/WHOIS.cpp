...
/*   Updated: 2023/03/31 17:06:38 by sel-mars         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void irc::commands::WHOIS( irc::client& client_ ) {
	...
	client_._msg_out += RPL_WHOISSERVER( client_, target );
	client_._msg_out += RPL_WHOISCHANNELS( client_, target );
	if ( target->_mode & UMODE_AWAY )
		client_._msg_out += RPL_AWAY( client_, target->_nickname, target->_away_msg );
	if ( target->_mode & UMODE_OPERATOR ) client_._msg_out += RPL_WHOISOPERATOR( client_, target );
