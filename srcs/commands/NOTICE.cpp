...
/*   Updated: 2023/03/22 15:09:44 by acmaghou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		if (client_._message._params.front().at(0) == '#') {
			...
			if (chan) {
				...
				if ( ( *it )->_nickname != client_._nickname )
					( *it )->_msg_out += NOTICE_MSG( client_, chan->_name, client_._message._params.back());
			}
		}
		else {
			irc::client* user = irc::server::__serv->findClient( client_._message._params.front() );
			if (user)
				user->_msg_out += NOTICE_MSG( client_, user->_nickname, client_._message._params.back() );
		}
	}
}